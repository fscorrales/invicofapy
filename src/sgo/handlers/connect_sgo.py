#!/usr/bin/env python3
"""

Author : Fernando Corrales <fscpython@gmail.com>

Date   : 21-feb-2026

Purpose: Connect to Sistema de Gestión de Obras (SGO) and download reports
"""

__all__ = [
    "ConnectSGO",
    "login",
    "logout",
    "go_to_report",
    "SGOReportManager",
]

import argparse
import asyncio
import datetime as dt
import io
import os
from abc import ABC, abstractmethod
from dataclasses import dataclass
from enum import Enum
from pathlib import Path

import pandas as pd
from playwright._impl._browser import Browser, BrowserContext, Page
from playwright.async_api import Download, Playwright, async_playwright


# --------------------------------------------------
def get_args():
    """Get command-line arguments"""

    parser = argparse.ArgumentParser(
        description="Connect to SGO and download reports",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument(
        "-u",
        "--username",
        help="Username for SGO access",
        metavar="username",
        type=str,
        default=None,
    )

    parser.add_argument(
        "-p",
        "--password",
        help="Password for SGO access",
        metavar="password",
        type=str,
        default=None,
    )

    args = parser.parse_args()

    if args.username is None or args.password is None:
        from ...config import settings

        args.username = settings.SGO_USERNAME
        args.password = settings.SGO_PASSWORD
        if args.username is None or args.password is None:
            parser.error("Both --username and --password are required.")

    return args


# --------------------------------------------------
@dataclass
class ConnectSGO:
    browser: Browser = None
    context: BrowserContext = None
    home_page: Page = None
    reports_page: Page = None


# --------------------------------------------------
class ReportCategory(str, Enum):
    SaldosBarriosEvolucion = "InformeEvolucionDeSaldosPorBarrio.aspx"
    Recursos = "SUB - SISTEMA DE CONTROL de RECURSOS"
    Contabilidad = "SUB - SISTEMA DE CONTABILIDAD PATRIMONIAL"
    Formulacion = "SUB - SISTEMA DE FORMULACION PRESUPUESTARIA"
    Clasificadores = "SUB - SISTEMA DE CLASIFICADORES"


# --------------------------------------------------
async def login(
    username: str, password: str, playwright: Playwright = None, headless: bool = False
) -> ConnectSGO:
    if playwright is None:
        playwright = await async_playwright().start()

    browser = await playwright.chromium.launch(
        headless=headless, args=["--start-maximized"]
    )
    context = await browser.new_context(no_viewport=True)
    page = await context.new_page()

    try:
        "Open SGV webpage"
        await page.goto("https://obras.invico.gov.ar/Account/Login")

        "Login with credentials"
        # Esperar a que el div de login esté presente
        await page.wait_for_selector("//div[@class='login']")

        # Interactuar con los elementos (Playwright espera automáticamente a que sean accionables)
        await page.fill("//input[@id='UserName']", username)
        await page.fill("//input[@id='Password']", password)

        # Hacer clic en el botón
        await page.click("//*[@id='formConEstilo']/form/button")

        # Espera a que el tráfico de red se detenga por completo
        await page.wait_for_load_state("networkidle")

        # Ahora verificamos que el elemento específico esté ahí
        await page.wait_for_selector("//*[@id='grillaObras']/tbody/tr[2]")

    except Exception as e:
        print(f"Ocurrio un error: {e}")

    return ConnectSGO(
        browser=browser, context=context, home_page=page, reports_page=None
    )


# --------------------------------------------------
async def go_to_report(connect: ConnectSGO, report: ReportCategory) -> None:
    pass
    # try:
    #     # New Tab generated
    #     report_page = "https://gv.invico.gov.ar/App/Recupero/Informes/" + report.value
    #     connect.reports_page = await connect.context.new_page()
    #     await connect.reports_page.goto(report_page)
    #     await connect.reports_page.wait_for_load_state("networkidle")
    # except Exception as e:
    #     print(f"Ocurrio un error: {e}")
    #     await logout(connect)


# --------------------------------------------------
async def go_back_to_reports_list(connect: ConnectSGO) -> None:
    pass
    # try:
    #     btn_volver = connect.reports_page.locator("xpath=//div[@id='pt1:btnVolver']")
    #     await btn_volver.click()
    #     await connect.reports_page.wait_for_load_state("networkidle")
    # except Exception as e:
    #     print(f"Ocurrio un error: {e}")
    #     await logout(connect)


# --------------------------------------------------
async def read_xls_file(file_path: Path) -> pd.DataFrame:
    """Read xls file"""
    try:
        with open(file_path, "rb") as f:
            file_bytes = f.read()
        # Convertir a DataFrame en memoria
        df = pd.read_excel(
            io.BytesIO(file_bytes),
            index_col=None,
            header=None,
            na_filter=False,
            dtype=str,
        )
        df.columns = [str(x) for x in range(df.shape[1])]
        return df

    except Exception as e:
        print(f"Error al leer el archivo: {e}")
        return None


# --------------------------------------------------
async def logout(connect: ConnectSGO) -> None:
    try:
        # 1. Abrir el menú popup (el equivalente al mnu_popup de Selenium)
        # Usamos click() directamente, Playwright esperará a que sea visible
        await connect.home_page.click("//*[@id='header']/ul[2]/li/div")

        # 2. Hacer clic en el botón de desconectar/cerrar sesión
        # No hace falta sleep(1), Playwright reintenta el click si el menú está animándose
        await connect.home_page.click("//*[@id='header']/ul[2]/li/div/ul/li[3]/a")

        # 3. Esperar a que la página de login vuelva a aparecer para confirmar el logout
        await connect.home_page.wait_for_selector("//div[@class='login']")

        # 4. Limpieza de recursos
        # Al cerrar el browser, se cierran automáticamente context y home_page
        await connect.browser.close()

        print("Sesión cerrada y navegador finalizado exitosamente.")

    except Exception as e:
        print(f"Ocurrió un error durante el logout: {e}")
        # Aseguramos el cierre incluso si falla el flujo visual
        if connect.browser:
            await connect.browser.close()
    # try:
    #     await connect.home_page.locator(
    #         "//*[@id='ctl00_UcPanelArribaUsuarioActual1_lblUsuario']"
    #     ).click()
    #     await connect.home_page.wait_for_load_state("networkidle")
    #     await connect.home_page.locator(
    #         "//*[@id='ctl00_UcPanelArribaUsuarioActual1_linkCerrar']"
    #     ).click()
    #     await connect.home_page.wait_for_load_state("networkidle")
    #     await connect.home_page.evaluate("window.location.reload()")
    #     await connect.context.close()
    #     await connect.browser.close()
    # except Exception as e:
    #     print(f"Ocurrio un error: {e}")


# --------------------------------------------------
@dataclass
class SGOReportManager(ABC):
    sgo: ConnectSGO = None
    download: Download = None
    df: pd.DataFrame = None
    clean_df: pd.DataFrame = None

    # --------------------------------------------------
    async def login(
        self, username: str, password: str, playwright: Playwright, headless: bool
    ) -> ConnectSGO:
        self.sgo = await login(
            username=username,
            password=password,
            playwright=playwright,
            headless=headless,
        )
        return self.sgo

    # --------------------------------------------------
    async def go_to_report(self, report: ReportCategory) -> None:
        await go_to_report(connect=self.sgo, report=report)

    # --------------------------------------------------
    @abstractmethod
    async def go_to_specific_report(self) -> None:
        """Go to specific report"""
        pass

    # --------------------------------------------------
    @abstractmethod
    async def download_report(
        self, ejercicio: str = str(dt.datetime.now().year)
    ) -> Download:
        """Download report from SIIF"""
        pass

    # --------------------------------------------------
    async def go_back_to_reports_list(self) -> None:
        await go_back_to_reports_list(connect=self.sgo)

    # --------------------------------------------------
    @abstractmethod
    async def process_dataframe(self, dataframe: pd.DataFrame = None) -> pd.DataFrame:
        """ "Transform read xls file"""
        pass

    # --------------------------------------------------
    async def read_xls_file(self, file_path: Path = None) -> pd.DataFrame:
        """Read xls file"""
        try:
            if file_path is None:
                if self.download is None:
                    raise ValueError(
                        "No se ha leído el archivo. 'self.download' es None."
                    )
                file_path = await self.download.path()
            self.df = await read_xls_file(file_path=file_path)
            return self.df

        except Exception as e:
            print(f"Error al leer el archivo: {e}")
            return None

    # --------------------------------------------------
    async def save_xls_file(self, save_path: Path, file_name: str) -> None:
        """Save xls file"""
        try:
            # Wait for the download process to complete and save the downloaded file somewhere
            if self.download is None:
                raise ValueError(
                    "No se ha descargado ningún archivo. 'self.download' es None."
                )

            # Definir la ruta de guardado
            file_path = os.path.join(save_path, file_name)

            # Si el archivo ya existe, eliminarlo antes de guardar el nuevo
            if os.path.isfile(file_path):
                os.remove(file_path)

            # Guardar el archivo descargado
            await self.download.save_as(file_path)
            print(f"Reporte descargado en: {file_path}")
        except Exception as e:
            print(f"Error al descargar el reporte: {e}")
            # await self.logout()

    # --------------------------------------------------
    async def select_report_module(self, module: ReportCategory) -> None:
        try:
            await self.sgo.reports_page.click(
                "xpath=//select[@id='pt1:socModulo::content']"
            )
            cmb_modulo = self.sgo.reports_page.locator(
                "xpath=//select[@id='pt1:socModulo::content']"
            )
            await cmb_modulo.select_option(value=module.value)
            await self.sgo.reports_page.wait_for_load_state("networkidle")

        except Exception as e:
            print(f"Error al seleccionar el módulo de reportes: {e}")

    # --------------------------------------------------
    async def select_specific_report_by_id(self, report_id: str) -> None:
        try:
            input_filter = self.sgo.reports_page.locator(
                "input[id='_afrFilterpt1_afr_pc1_afr_tableReportes_afr_c1::content']"
            )
            await input_filter.clear()
            await input_filter.fill(report_id)
            await input_filter.press("Enter")
            btn_siguiente = self.sgo.reports_page.locator(
                "div[id='pt1:pc1:btnSiguiente']"
            )
            await self.sgo.reports_page.wait_for_load_state("networkidle")
            await btn_siguiente.click()
        except Exception as e:
            print(f"Error al seleccionar el módulo de reportes: {e}")

    # --------------------------------------------------
    async def logout(self) -> None:
        await logout(connect=self.sgo)


# --------------------------------------------------
async def main():
    """Make a jazz noise here"""

    args = get_args()

    async with async_playwright() as p:
        connect_sgo = await login(
            args.username, args.password, playwright=p, headless=False
        )
        # await go_to_reports(connect=connect_sgo)
        await logout(connect=connect_sgo)


# --------------------------------------------------
if __name__ == "__main__":
    asyncio.run(main())
    # From /invicofapy
    # poetry run python -m src.sgo.handlers.connect_sgo
