...
def login(username: str, password: str) -> ConnectSSCC:
    ...
    try:
        ...
        if not main.is_maximized():
            main.maximize()
        cmb_user = main.child_window(auto_id="1", control_type="ComboBox").wait('exists enabled visible ready')
        input_password = main.child_window(auto_id="2", control_type="Edit").wrapper_object()
        cmb_user.type_keys(username)
        input_password.type_keys(password)
        btn_accept = main.child_window(title="Aceptar", auto_id="4", control_type="Button").wrapper_object()
        btn_accept.click()
...
if __name__ == "__main__":
    ...
    main()
    # From /invicofapy
    # poetry run python -m src.sscc.handlers.connect_sscc
